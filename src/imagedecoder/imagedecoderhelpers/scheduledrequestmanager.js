...
module.exports = ScheduledRequestManager;

function ScheduledRequestManager(fetcher, scheduler) {
...
ScheduledRequestManager.prototype.createRequest = function createRequest(
    imagePartParams,
    contextVars,
    callback,
    terminatedCallback,
...
ScheduledRequestManager.prototype.createMovableRequestHandle = function createMovableRequestHandle() {
    var terminatedDummyCallback = dummyCallback;
    
    var internalRequestVars = createInternalRequestVars(
        this,
        /*imagePartParams=*/null,
        /*contextVars=*/null,
        dummyCallback,
    ...
    internalRequestVars.requestContext =
        this._fetcher.createMovableRequest(
            fetchCallback,
            internalRequestVars);
...
ScheduledRequestManager.prototype.moveRequest = function moveRequest(
    movableRequestHandle, newImagePartParams) {
    
    var internalRequestVars = this._movableRequestHandles[
        movableRequestHandle];
    
    internalRequestVars.imagePartParams = newImagePartParams;
...
ScheduledRequestManager.prototype.manualAbortNonMovableRequest = function manualAbortNonMovableRequest(
    requestId) {
...
ScheduledRequestManager.prototype.getContextVars = function getContextVars(requestId) {
    var internalRequestVars = this._nonMovableRequestById[requestId];
    if (internalRequestVars === undefined) {
        // This situation might occur if request has been terminated,
        // but user's terminatedCallback has not been called yet. It
...
function startNonMovableRequest(resource, internalRequestVars, movableRequestHandle) {
    ...
    if (internalRequestVars.isManuallyAborted) {
        if (resource !== null) {
            internalRequestVars.self._scheduler.jobDone(resource, internalRequestVars);
        }
    ...
    internalRequestVars.requestContext =
        internalRequestVars.self._fetcher.createProgressiveDataRequest(
            internalRequestVars.imagePartParams,
            fetchCallback,
            internalRequestVars,
            options);
...
function fetchCallback(requestContext, internalRequestVars) {
    try {
        if (internalRequestVars.isYielded || internalRequestVars.isTerminated) {
